...
BYTE command_parse(){ // parses the shell commands
        ...
        if(strcmp($keyboard_write_buffer,"mkdir") == 0){
                WORD name = fmalloc(strlen(str_arr_ind($keyboard_write_buffer,1))+1);
                strcpy(name,str_arr_ind($keyboard_write_buffer,1));
                append_folder(name);
                asm_d("JMP #end_command_parse");
        }
        if(strcmp($keyboard_write_buffer,"ls") == 0){
            ls();
            asm_d("JMP #end_command_parse");
        }
        ...
        if(strcmp($keyboard_write_buffer,"cd") == 0){
                WORD cur_fp_p = 0;
                WORD name = 0; // name of folder
                de = 0;
                if(strcmp(str_arr_ind($keyboard_write_buffer,1),"p") == 0){
                        current_directory = (struct folder_node) current_directory.parent;
                        asm_d("JMP #cd_not_else");
                }
       
                cur_fp_p = (struct folder_node) current_directory.contents+de;
                name = (struct folder_node) (WORD) *cur_fp_p .name;
       
       
       
       
                while(strcmp(name,str_arr_ind($keyboard_write_buffer,1)) != 0){
                    de = de +4;
                    cur_fp_p = (struct folder_node) current_directory.contents+de;
                    name = (struct folder_node) (WORD) *cur_fp_p .name;
                   
                }
                current_directory = (WORD) *cur_fp_p;
                asm_d("LABEL cd_not_else");
                asm_d("JMP #end_command_parse");
        }
